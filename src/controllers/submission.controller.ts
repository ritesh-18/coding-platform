...
export class SubmissionController{
    constructor(private problemservice:SubmissionService){}
    async createProblem(req:Request, res:Response , next:NextFunction){
        try {
            ...
            sendError(res, error, "Error while Submitting new Problem")
        }
    }
    async fetchProblem(req:Request ,res:Response ,next:NextFunction ){
        try {
            ...
            res.status(200).json({
                message:"Submited problem fetched successfully",
                success:true,
                data:response
            })
        } catch (error) {
            sendError(res, error, "Error while fetching a submitted Problem")
        }
    }
    ...
    async updateProblem(req:Request , res:Response ,next:NextFunction){
        try {
            ...
            res.status(200).json({
                message:"Submited problem status updated successfully",
                success:true,
                data:response
            })
        } catch (error) {
            sendError(res, error, "Error while updating status")
        }
    }
    async deleteProblem(req:Request , res:Response ,next:NextFunction){
        try {
            ...
            sendError(res, error, "Error while deleting a submited Problem")
        }
    }
}